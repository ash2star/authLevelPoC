...
hdbuserstore set KEY "${HANA_HOST}:${HANA_PORT}" "${HANA_USER}" "${HANA_PASSWORD}"

#create workspace local on the Jenkins server
regi create workspace regi_workspace --key=KEY -f
...
# Run tests
curl -u ${HANA_USER}:${HANA_PASSWORD} --cacert /etc/ssl/certs/hxehostqa.mooo.com.crt "https://${HANA_HOST}:${XS_PORT}/sap/hana/testtools/unit/jasminexs/TestRunner.xsjs?format=json&package=de.linuxdozent.gittest.test" -o test.json
...
regi show du ${DELIVERY_UNIT} || regi create du ${DELIVERY_UNIT} --key=KEY
...
regi assign ${PACKAGE} ${DELIVERY_UNIT} --key=KEY

# Full DU export 
mkdir ${WORKSPACE}/target
regi export ${DELIVERY_UNIT} "${WORKSPACE}/target/${DELIVERY_UNIT}.tgz" --key=KEY
